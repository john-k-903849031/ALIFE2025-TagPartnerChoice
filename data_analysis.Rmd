---
title: "tag_matching_alifeversion"
...
a <- read.csv(paste0(filepath,"org_dump_file.dat"),h=T,
...
b <- read.csv(paste0(filepath,"org_dump_file.dat"),h=T,
                                    #seed,tag_dist,tag_mut
                        colClasses=c("factor","factor","factor",
                                     #host_int,sym_int
                                    "double","double",
...
```{r}
what <- subset(small, tag_dist == '0.625' & tag_mut=='0.1')


what <- dplyr::select(what, c(tag_dist, tag_mut, tag_distance, seed, cond))
write.csv(what,"td0.625_tm0.1_pooled.csv", row.names=F)
?write.csv
read.csv("td0.625_tm0.1_pooled.csv")
?wilcox.test
wilcox.test(tag_distance ~ cond, data=what)
ggplot(what, aes(x=tag_distance)) + 
  geom_histogram(data=subset(what, sym_bin=="-1 to -0.6 (Parasitic)"), aes(fill=sym_bin),alpha=0.7,  bins=23)+
  geom_histogram(data=subset(what, sym_bin=="0.6 to 1.0 (Mutualistic)"), aes(fill=sym_bin), alpha=0.7, bins=23) +
  geom_vline(data=subset(what, sym_bin=="0.6 to 1.0 (Mutualistic)"), aes(xintercept = td_mean), color="#7A0403FF",linetype="dotted") +
  geom_vline(data=subset(what, sym_bin=="-1 to -0.6 (Parasitic)"), aes(xintercept = td_mean), color="#3E9BFEFF",linetype="dotted") +
  ...
  ylab("Pooled count across replicates")+
  facet_nested("Tag permissiveness" + tag_dist ~ "Tag mutation rate" + tag_mut, drop=TRUE) + 
  facet_nested_theme

ggplot(what, aes(x=cond,y=tag_distance, color=cond)) + geom_boxplot()+ 
  xlab("Tag proportional mismatch") + 
  ylab("Pooled count across replicates")+
  facet_nested("Tag permissiveness" + tag_dist ~ "Tag mutation rate" + tag_mut, drop=TRUE) + 
     guides(fill="none")

Summarize(tag_distance ~ cond,
          data=what,
          digits=3)
install.packages("FSA")co
library(FSA)

a <- rbind(data.frame(cond = rep("para", 1000), tag_distance = sample(subset(what, cond=="para")$tag_distance, 1000, replace = T)),data.frame(cond = rep("mut", 1000), tag_distance = sample(subset(what, cond=="mut")$tag_distance, 1000, replace = T)))
a
?sample
rep("para", 1000)
sample(subset(what, cond=="para")$tag_distance, 5, replace = T)
sample()

wilcox.test(tag_distance ~ cond, data=what)$p.value

?t.test()

t.test(tag_distance~ cond, data=what)

```

### plot histogram of pair tag mismatch
...
ggplot(small, aes(x=tag_distance)) + 
  geom_histogram(data=subset(small, sym_bin=="-1 to -0.6 (Parasitic)"), aes(fill=sym_bin),alpha=0.7,  bins=16)+
  geom_histogram(data=subset(small, sym_bin=="0.6 to 1.0 (Mutualistic)"), aes(fill=sym_bin), alpha=0.7, bins=16) +
...
### wilcoxon & cliff's delta for difference between para and mut

```{r}
colnames(tag_dist_df)
small <- subset(small) # get the merged df of sym-host paris between para and mut conditions

small <- small %>% group_by(tag_dist,tag_mut) %>% mutate(para_pres = "para"%in%cond)
small <- subset(small, para_pres == T & tag_mut != '0')
...
for (td in unique(small$tag_dist)){
  for(tm in unique(small$tag_mut)){
    a <- subset(small, tag_dist == td & tag_mut == tm)
    if("para" %in% a$cond){
      n_test <- n_test + 1
      print(paste0("td",td,"_tm",tm))
      nm <- append(nm, paste0("td",td,"_tm",tm))
...
models <- data.frame(names = nm, p_vals=md, deltas=)
p_val_to_beat <- 0.05 / n_test

what$cond <- as.factor(what$cond)
wilcox.test(tag_distance ~ cond, data=what)$p.value
library(effsize)

package.install("effsize")
install.packages("effsize")

str(what$cond)

thing <- cliff.delta(tag_distance ~ cond, data=what)
str(thing)$estimate

thing$estimate
?cliff.delta
# small_small currently contains the combo of par
```

### count of surviving replicates

```{r}
filepath<- "../../symbulation_experiments/2025.05.02_control_fixed_mutualists/"
a <- read.csv(paste0(filepath,"sym_vals.dat"),h=T)
a$cond <- "mut"

filepath<- "../../symbulation_experiments/2025.05.02_control_fixed_parasites/"
b <- read.csv(paste0(filepath,"sym_vals.dat"),h=T)
b$cond <- "para"

sym_vals_df <- rbind(a,b)
  
identifying_cols <- 1:3
for (x in identifying_cols){
  sym_vals_df[[x]]<- as.factor(sym_vals_df[[x]])
}

small <- subset(sym_vals_df, update == max(update))
small$survived <- small$count > 0
small <- small %>% group_by(tag_dist,tag_mut,cond) %>% summarise(survivors = sum(survived))
nrow(subset(small, survivors > 0 & tag_mut != '-1'))
subset(small, survivors > 0 & survivors < 30 & cond=="para")
ggplot(small, aes(x=tag_mut, y=tag_dist,fill=survivors)) + geom_tile() + facet_wrap(~cond)
...
levels(hosts_df$tag_mut)
hosts_df$tag_mut <- factor(hosts_df$tag_mut, levels=c("no tag", "0", "1e-04", "5e-04", "0.001", "0.005", "0.01", "0.05", "0.1"))
syms_df$tag_mut <- factor(syms_df$tag_mut, levels=c("no tag", "0", "1e-04", "5e-04", "0.001", "0.005", "0.01", "0.05", "0.1"))
